---
title: "| <span style='font-size: 30px'>Full workflow:\n| Trophic asynchrony and climate change\n"
subtitle: "Steps 1-4 in a Bayesian framework
\n From: A four-step simulation-based workflow for ecology"
pagetitle: Full workflow
output:
...
y ~ normal(ypred, sigma_y); // this creates an error model that is normally distributed
// Priors ...
mu_a ~ normal(100,30);
sigma_a ~ normal(0,20);
mu_b ~ normal(0,5);
...
muparams <- sumer[grep("mu", rownames(sumer)), 
    c("mean", "2.5%", "25%", "50%", "75%", "97.5%")]
sigmaparams <- sumer[grep("sigma", rownames(sumer)), 
    c("mean", "2.5%","25%", "50%", "75%", "97.5%")]
...
plot(spslopes~species_trend, xlab="Given species-level slopes", 
    ylab="Modeled species-level slopes", col="darkblue")
abline(0,1)
```
...
year <- d$yr1981
syncmodelhis <- stan("stan/twolevelhierslopeint.stan", 
    data=c("N","Nspp","y","species","year"),
...
# For this version, let's use the species-level intercepts and slopes 
# (e.g., syncmodelhispost[["b"]]) ...
for (i in 1:simshere){
